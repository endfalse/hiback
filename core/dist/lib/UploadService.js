...
export default class UploadService {
    constructor(requestFactoryInstance) {
        this.uploadedChunks = 0;
        //累加
        this._nextUid = 1;
        this.getUploadLoaded = (completedChunks, size) => {
    ...
    upload(file, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let ret;
            if (opts.chunk !== true) {
                ret = yield this.normalUpload(requestOpts);
            }
            else {
                ret = yield this.uploadChunks({
                    method: 'post',
                    file: file,
                    data: opts.data,
                    headers: opts.headers,
    ...
    * */
    ...
    getUploadRequestHandler() {
        ...
        return (opts) => __awaiter(this, void 0, void 0, function* () {
            ...
            const ret = yield this.upload(opts.file, {
                data: opts.data,
    ...
    getUploadId(meta_1, data_1, headers_1) {
        return __awaiter(this, arguments, void 0, function* (meta, data, //Record<string, string | Blob | [string | Blob, string]>,
        headers, api = undefined) {
            ...
            const uploadId = yield this.requestFactoryInstance.request({
    ...
    normalUpload(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const response = yield this.requestFactoryInstance.request({
    ...
    uploadChunks(options) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                const id = uuidv4();
                let finalResponse; // 收集最终有效结果
                try {
                    // 获取文件MD5
                    const md5 = yield getFileMd5(id, options.file, this.uploadNotify);
                    // 每批并发数（可配置）
                    ...
                    // 获取上传 ID
                    const uploadId = yield this.getUploadId(meta, options.data, options.headers, options.api);
                    ...
                    for (let i = 0; i < totalChunks; i += batchSize) {
                        ...
                        // 构建当前批次的上传 Promise
                        const batchPromises = batchChunkNumbers.map((chunkNumber) => __awaiter(this, void 0, void 0, function* () {
                            ...
                            const currentResponse = yield this.uploadChunkWithRetry(Object.assign(Object.assign({ id, uploadId, chunkNumber, chunkBlob }, meta), { completedChunks }), options.data, options.headers, options.api, this.config.maxRetries, this.config.retryDelay);
                        ...
                        if (failedChunks.length > 0) {
                            ...
                            // 提取失败原因，方便排查
                            const failReasons = failedChunks.map((res) => res.reason).join('; ');
    ...
    uploadChunkWithRetry(chunkParams_1, data_1, headers_1, api_1) {
        return __awaiter(this, arguments, void 0, function* (chunkParams, data, headers, api, maxRetries = 3, // 默认最大重试3次
        retryDelay = 1000 // 重试间隔1秒（可选，避免频繁重试）
        ) {
            ...
            while (attempt < maxRetries) {
                try {
                    // 尝试上传单个分片
                    const response = yield this.uploadChunk(chunkParams, data, headers, api);
                    // 上传成功：返回结果
                    return response;
                }
                catch (error) {
    ...
    uploadChunk(upMetadata_1, data_1, headers_1) {
        return __awaiter(this, arguments, void 0, function* (upMetadata, data, headers, api = undefined) {
            this.uploadedChunks++;
            // 生成分片MD5（使用js-md5库）
            const chunkMd5 = yield getBlobMd5(upMetadata.chunkBlob);
    ...
    getResponseData(id, response) {
        // 封装：解析单个 response 项为 ComplexUploaded
        const parseSingleItem = (itemResponse) => {
            ...
            const { accessUrl, thumbnailUrl, uniqueId, width, height, posterUrl, duration, taskId } = itemResponse.file || {};
            // 3. 修复类型判断错误 + 简化分支逻辑
            // 基础文件（仅核心字段）
            if (uniqueId && accessUrl && thumbnailUrl) {
                return { id, uniqueId, accessUrl, thumbnailUrl, taskId };
            }
            // 图片（有宽高，无时长/海报）
            if (uniqueId && accessUrl && thumbnailUrl &&
                typeof width === 'number' && typeof height === 'number' &&
                typeof duration === 'undefined' && !posterUrl) {
                return { id, uniqueId, accessUrl, thumbnailUrl, width, height, taskId };
            }
            // 视频（有海报/时长，无缩略图）
            if (uniqueId && accessUrl && posterUrl &&
                typeof width === 'number' && typeof height === 'number' &&
                typeof duration === 'number' && !thumbnailUrl) {
                return { id, uniqueId, accessUrl, posterUrl, width, height, duration, taskId };
            }
            return undefined;
        };
        ...
        if (Array.isArray(response)) {
            const parsedList = response.map(parseSingleItem).filter(Boolean);
