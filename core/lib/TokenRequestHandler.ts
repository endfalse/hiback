...
export default class TokenRequestHandler<TResponseCode=number> {
  ...
  config = {
    ...
    reSendRequest: <T=any,D=any>(config: AxiosRequestConfig<D>,reslove:(response:AxiosResponse<T, D, {}>)=>void,reject:(error:AxiosError<T, D>)=>void)=>{
      axios(config)
  ...
  private requestServer:RequestService<TResponseCode>
  ...
  private getAxiosError(response?: AxiosResponse | undefined){
    return new AxiosError('刷新令牌失败，请重新登录','REFRESH_TOKEN_FAILED',
        undefined,
        undefined,
        response
    )
   ...
   */
  async handleRequestError(error: AxiosError): Promise<AxiosResponse> {
  ...
  private async refreshTokenAndRetry(): Promise<void> {
    ...
    // 1. 执行刷新令牌（使用者实现的逻辑）
    const latestToken = await this.config.refreshToken();
